...
class ModelConfig:
    ...
    def download_if_necessary(self, use_usp=False):
        if self.path is None:
            ...
            target_dir = os.path.join(self.local_model_path, self.model_id)
            existing_files = glob.glob(self.origin_file_pattern, root_dir=target_dir) if os.path.isdir(target_dir) else []

            if existing_files:
                print(f"[ModelConfig] Found existing files for {self.model_id} at {target_dir}, skipping download.")
            elif not skip_download:
                print(f"[ModelConfig] Downloading {self.model_id} to {target_dir} ...")
                snapshot_download(
                    self.model_id,
                    local_dir=target_dir,
                    allow_patterns=allow_file_pattern,
                    ignore_patterns=existing_files if existing_files else None,
                )
